...
export class CppBridge {
   ...
   */
  async isValidAddress(address: string, nettype: number): Promise<boolean> {
  ...
  async deleteWallet(walletId: string, backend: WalletBackend): Promise<void> {
    const response = await this.module.callMonero('deleteWallet', [
      this.module.documentDirectory,
      walletId,
      backend
    ])
  ...
  async getAllTransactions(
    ...
    return parsed as TransactionsPage
  }
  ...
  async createTransaction(
    ...
    return parsed as SignedTransaction
  }
